...
set -eu
echo "== repo"
gh api repos/sjh9714/repo-cover --jq '"stars=\(.stargazers_count) forks=\(.forks_count) watchers=\(.subscribers_count) issues=\(.open_issues_count)"'
echo "== repo issues/PRs"
gh api 'repos/sjh9714/repo-cover/issues?state=all&per_page=10' --jq '.[] | "\(.number) \(.state) \(.comments)c \(.title)"' 2>/dev/null || true
echo "== list PRs"
