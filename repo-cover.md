...
1. Gather the target repo owner, name, description, primary
...
2. Rewrite the description into one or two sentences, max 110 characters
   with a 60-character maximum for CJK. Use concrete nouns and no marketing adjectives.
3. Write `<repo>-cover.html` from the skeleton below.
4. Export the PNG with this command.
   ...
   The macOS binary is `/Applications/Google Chrome.app/Contents/MacOS/Google Chrome`.
...
ink until it reaches 4.5:1 contrast on paper. With no primary language, use a
...
- Title size follows name length. Use up to 9 chars 132px, up to 14 108px, up to
  20 92px, up to 26 74px, longer 64px on two lines broken at a hyphen.
- For the arcs, hash is the first 8 hex digits of sha256(repo name) as an integer.
  ...
  outward use .5 .36 .26 .17 .11 .07. The opaque meta strip must cover them
  at the rule.
- Use one accent color in exactly four places, the eyebrow dot, title period, arc
  strokes, meta language dot.
- Keep the description to 2 lines. Wrap compound words in
  `<span style="white-space:nowrap">` so hyphens never break mid-word.
- Korean text uses 30px, line-height 1.6, and `word-break:keep-all`. Japanese
  and Chinese use default breaking and swap the Google Fonts family to
...
- Never use box-shadow, drop-shadow, gradients, glassmorphism, or emoji.
