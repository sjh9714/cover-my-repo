...
`.win` frame inset 72px vertical, 96px horizontal, radius 10:

1. `.bar`, 44px, three 12px `.dot` circles, centered `.wtitle`
   reading `owner/name &mdash; zsh` in mono 13px `dim`. Keep the class
...
5. `.status`, bottom strip inside the window: metas joined with
   `&middot;` uppercase on the left, `EXIT 0` on the right.

## Rules

- Mono everywhere, no serif, no sans.
- Accent appears exactly twice: prompt symbol, cursor block.
